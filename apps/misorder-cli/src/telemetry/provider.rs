...
pub enum LogSink {
    #[default]
    Stdout,
    /// For an invocation whose stdout carries data instead of text. `mis run
    /// --trace -` writes JSON Lines there, and a log line in the middle of a
    /// trace makes the file unparseable at some byte offset that says nothing
    /// about the cause.
    Stderr,
}
