...
/// JUnit XML, for CI.
///
/// The format every CI system already renders, so a failing scenario shows up
/// where a failing test does rather than in a log a human has to open. A shrunk
/// trace committed next to it is not a flaky test: it is an exact replayable
...
pub mod junit {
    ...
    pub fn render(suite: &str, cases: &[Case]) -> String {
        ...
        out.push_str(&format!(
            "<testsuites name=\"misorder\" tests=\"{}\" failures=\"{failures}\" time=\"{total:.3}\">\n",
            cases.len()
        ));
        out.push_str(&format!(
    ...
    ///
    /// A reproducer carries SQL and NATS subjects, so `<`, `>` and `&` all turn
    /// up in normal output. An unescaped one produces a report file that every
    /// CI system silently refuses to parse, and the run then looks like it
    /// passed.
    ...
    mod tests {
        ...
        #[test]
        fn a_passing_case_is_a_self_closing_element() {
            let xml = render(
                "dead_letter_no_redelivery",
                &[Case {
        ...
        #[test]
        fn subjects_with_angle_brackets_are_escaped() {
            let reproducer = crate::report::tests::sample_reproducer();
            ...
            assert!(
                !xml.contains("ledger.>"),
                "an unescaped subject makes the report unparseable"
            );
            assert!(xml.contains("ledger.&gt;"), "{xml}");
        }
    }
}
